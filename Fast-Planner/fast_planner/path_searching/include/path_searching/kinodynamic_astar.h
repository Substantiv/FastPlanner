...
// #define REACH_END 2
// #define NO_PATH 3
#define IN_CLOSE_SET 'a'        // 节点属于闭集
#define IN_OPEN_SET 'b'         // 节点属于开集
#define NOT_EXPAND 'c'          // 节点没有被扩展
#define inf 1 >> 30             // 无穷大

// 在Fast planner的实现中，open集是通过两个数据结构实现的，一个队列用来存储，弹出open集中的节点。
...
class PathNode {
 public:
  /* -------------------- */
  Eigen::Vector3i index;                // 当前节点在栅格地图中的索引
  Eigen::Matrix<double, 6, 1> state;    // 节点的状态(位置,速度)
  double g_score, f_score;              // 评分（f=g+h）
  Eigen::Vector3d input;                // 输入
  double duration;
  double time;                          // dyn
  int time_idx;                         // 时间索引
  PathNode* parent;                     // 父节点
  char node_state;                      // 节点状态(闭集、开集、未被扩展)

  /* -------------------- */
  // 构造函数,节点初始状态为无父节点且未被扩展
  PathNode() {
    parent = NULL;
...
/*
 *哈希表NodeHashtable 用来查询节点是否已经存在于open集中
 */
class NodeHashTable {
 private:
  /* data */
...
class KinodynamicAstar {
 private:
  /* ---------- main data structure ---------- */
  vector<PathNodePtr> path_node_pool_;          // 存储规划路径中的所有节点
  int use_node_num_, iter_num_;                 // 使用节点个数和迭代次数
  NodeHashTable expanded_nodes_;                // 被扩展过的节点

  // open集是通过两个数据结构实现的，一个队列用来存储弹出open集中的节点
  // 另一个哈希表NodeHashtable 用来查询节点是否已经存在于open集中。
